...
Configurator digunakan untuk mengatur aplikasi.

config.add_route('hello', '/') mendaftarkan bahwa URL / akan memiliki nama rute 'hello'.

config.add_view(hello_world, route_name='hello') memberitahu Pyramid: "Jika ada yang mengakses rute 'hello', jalankan fungsi hello_world".

make_wsgi_app() membuat aplikasi WSGI yang bisa dimengerti oleh server.

make_server dari wsgiref (bawaan Python) digunakan untuk menjalankan aplikasi di port 6543.

Masalah/Error: (Tulis "Tidak ada masalah. Program berjalan lancar." jika memang lancar)
